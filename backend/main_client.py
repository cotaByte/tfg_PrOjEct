...
def login():
    ...
    if request.method == "POST":
        ...
        req = requests.post('http://'+host+':5000/login',
...
def menu():
    ...
    if request.method == "GET":
        if (token != None):
            ...
            headers = {'Content-Type': 'application/json'}
            req = requests.get('http://'+host+':5000/greetings',
                               json=data, verify=False, headers=headers)
            array = req.json()
            nombre = array['nombre']
...
def register():
    ...
    if request.method == "POST":
        data = auxMethods.setObject4Server(
            request.form, "nif", "nombre", "apellido1", "apellido2", "instrumento", "tlf", "pin")
        headers = {'Content-Type': 'application/json'}
        req = requests.post('http://'+host+':5000/addUser',
                            json=data, verify=False, headers=headers)
...
def registerUser():
    ...
    if (token == None):
        ...
        if request.method == "POST":
            ...
            # Added param ok message to retrieve the ok message from the server in the html
...
def registerBanda():
    ...
    if (token == None):
        ...
        if (request.method == 'POST'):
            data = auxMethods.setObject4Server(
                request.form, "nombre", "poblacion")
            headers = {'Content-Type': 'application/json'}
            req = requests.post('http://'+host+':5000/addBanda',
                                json=data, verify=False, headers=headers)
            array = req.json()
            ok = array.get('ok')
            data = array.get('data')
...
def joinner():
    ...
    if (token != None):
        if request.method == "GET":
            data = {"token": token}
            headers = {'Content-Type': 'application/json'}
            req = requests.get('http://'+host+':5000/listJoin',
                               json=data, verify=False, headers=headers)
            llista = req.json()
        ...
        if request.method == "POST":
            ...
            headers = {'Content-Type': 'application/json'}
            req = requests.post('http://'+host+':5000/join',
                                json=data, verify=False, headers=headers)
...
def leaver():
    ...
    if (token != None):
        if request.method == "GET":
            ...
            data = {"token": token}
            headers = {'Content-Type': 'application/json'}
            req = requests.get('http://'+host+':5000/listLeave',
                               json=data, verify=False, headers=headers)
            llista = req.json()
        ...
        if request.method == "POST":
            ...
            headers = {'Content-Type': 'application/json'}
            req = requests.post('http://'+host+':5000/leave',
                                json=data, verify=False, headers=headers)
...
def addEvent():
    ...
    if (token == None):
        ...
        if (request.method == 'POST'):
            data = auxMethods.setObject4Server(
                request.form, "nombre", "ubicacion", "fecha_evento")
            headers = {'Content-Type': 'application/json'}
            req = requests.post('http://'+host+':5000/addEvent',
                                json=data, verify=False, headers=headers)
            array = req.json()
            ok = array.get('ok')
            data = array.get('data')

            # Added param ok message to retrieve the ok
